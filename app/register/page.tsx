      ...
      if (nickNameCheck.docs.length == 0) {
        await createUserWithEmailAndPassword(authService, `${email}@email.daelim.ac.kr`, password)
          .then((value) => {
            ...
            setDoc(doc(db, 'chat', value.user.uid), {
              'daelimmarket': [
                {
                  'send_time': Date.now(),
                  'sender': 'daelimmarket',
  ...
  return (
    <article className={style.article}>
      <div className={style.form}>
        ...
        <div className={style.formDiv}>
          <input className={style.input} onChange={(event) => { setNickname(event.target.value); }} placeholder="닉네임(2-8자)" type="text" maxLength={8} />
        </div>
        <div className={style.formDiv}>
          <input className={style.input} onChange={(event) => { setPassword(event.target.value); }} placeholder="비밀번호(4자 이상)" type="password" />
        </div>
        <div className={style.formDiv}>
          <input className={style.input} onChange={(event) => { setConfirm(event.target.value); }} placeholder="비밀번호 확인" type="password" />
        </div>
        ...
        <button onClick={onSubmit} className={style.button}>회원가입</button>
      </div>
    </article>
  );
}