...
import { DocumentData, arrayRemove, arrayUnion, deleteDoc, doc, getDoc, updateDoc } from "firebase/firestore";
...
export default function Detail({ params }: { params: { id: string } }) {
  ...
  const getDocument = async () => {
    try {
      const productDocument = (await getDoc(doc(db, "product", params.id))).data();
  ...
  const deletePost = async () => {
    try {
      ...
      await Promise.all((await listAll(ref(storage, `product/${params.id}`))).items.map(async (item) => {
            ...
            <>
              <div className={style.detailTopDiv}>
                <div className={style.alignImgDiv}>
                  <div className={style.productImgDiv} style={{ marginLeft: `${-100 * dotIndex}%` }}>
                    {document["images"].map((value: string, index: number) => { return <img className={style.productImg} src={value} key={index} alt="상품 이미지" /> })}
                    {
                      parseInt(document["status"]) > 0 ?
                        <div className={style.statusImgDiv}>
                          {parseInt(document["status"]) == 1 ? <p>예약중</p> : <p>판매완료</p>}
                        </div> :
                        <></>
                    }
                  </div>
                  <div className={style.dots}>
                    {document["images"].map((_: string, index: number) => { return <div className={style.dot} onClick={() => setIndex(index)} style={{ backgroundColor: index == dotIndex ? "#FFFFFF" : "#ACACAC" }} key={`dot${index}`}></div> })}
                  </div>
                </div>
                <div className={style.infoDiv}>
                    ...
                    <p className={style.uploadtime}>등록일: {format(document["uploadTime"].toDate(), 'yyyy년 MM월 dd일 aa h시 mm분 ss초', { locale: ko })}</p>
                    {
                      document["uid"] == uid ?
                        <span className={style.delete} onClick={() => {
                          const confirm = window.confirm(`정말로 판매글을 삭제하시겠습니까?`);
                          if (confirm) {
                            deletePost();
                          }
                        }}>삭제하기</span> :
                  ...
                  <div className={style.buttonsDiv}>
                    <div className={style.likeDiv}
                      ...
                      <Image className={style.likeImg} src={document["likes"].includes(uid) ? "/images/detail/icon_heart_fill.svg" : "/images/detail/icon_heart.svg"} alt="관심 아이콘" width={33} height={29} />
                    ...
                    {
                      document["uid"] == uid && parseInt(document["status"]) < 2 ?
                        <div className={style.statusDiv}>
                          <select className={style.select} value={status}
                            onChange={(event) => {
                              const confirm = window.confirm(`정말로 ${event.target.value} 상태로 바꾸시겠습니까?`);
                              if (confirm) {
                                changeStatus(event);
                              }
                            }}>
